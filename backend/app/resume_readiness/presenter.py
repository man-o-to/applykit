...
def _category(score: int | None, band: str | None) -> ResumeReadinessCategoryResponse | None:
    if score is None or band is None:
        return None
    return ResumeReadinessCategoryResponse(score=score, band=band)
...
def present_analysis(analysis: ResumeReadinessAnalysis) -> ResumeReadinessResponse:
    ...
    summary = ResumeReadinessSummaryResponse(
        critical=sum(item.severity == "critical" and item.outcome in {"fail", "warning"} for item in findings),
        important=sum(item.severity == "important" and item.outcome in {"fail", "warning"} for item in findings),
        improvements=sum(item.severity == "improvement" and item.outcome in {"fail", "warning"} for item in findings),
    ...
    return ResumeReadinessResponse(
        ...
        categories=ResumeReadinessCategoriesResponse(
            parseability=_category(analysis.parseability_score, analysis.parseability_band),
            quality=_category(analysis.quality_score, analysis.quality_band),
            tailoring=_category(analysis.tailoring_score, analysis.tailoring_band),
        ),
        summary=summary,
        findings=findings,
